...
按 Change Class 选对应 gate。所有 gate 都要求 **Observe** 先行（HEAD + clean tree + runtime state）。

## C0 — 文案 / 图片 / CSS 微调

- [ ] 改动在 approved scope
- [ ] 涉及文件 `python -m py_compile` / XML 解析无错
- [ ] targeted smoke：相关页面 HTTP 200，视觉无破损
- [ ] commit（不强制 full install）

## C1 — QWeb 页面 / 菜单 / 表单

- [ ] module `-u trade_website` upgrade EXIT=0
- [ ] 受影响 route HTTP 200（含 `/en_US/` 与 `/zh_CN/`）
- [ ] browser：导航 / 菜单层级 / 表单渲染正确
- [ ] 双语各验证一次
- [ ] commit + push

## C2 — schema / CRM 字段 / business logic

- [ ] 新 migration 文件含 Purpose / From / Invariant / Destructive / Rollback（见 GOVERNANCE §7）
- [ ] **clean install** 从 baseline 起 EXIT=0
- [ ] `-u` upgrade replay 通过
